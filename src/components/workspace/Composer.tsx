...
export default function Composer({
  ...
  const addFiles = async (files: File[]) => {
    ...
    if (unsupported.length) {
      alertDialog(t('composer.unsupported_files', { names: unsupported.map((f) => f.name).join('、') }))
    }
    if (imgFiles.length) {
      const imgs = (await Promise.all(imgFiles.map(readImage))).filter((x): x is Img => !!x)
      if (imgs.length < imgFiles.length) alertDialog(t('composer.images_failed'))
    ...
    for (const f of pdfs) {
      ...
      try {
        ...
        if (doc.text.trim().length < 20) {
          alertDialog(t('composer.pdf_no_text', { name: f.name }))
        } else {
          setDocs((prev) => [...prev, { ...doc, id: Math.random().toString(36).slice(2) }])
        }
      } catch (e) {
        const stack = e instanceof Error && e.stack ? '\n' + e.stack.split('\n').slice(0, 2).join('\n') : ''
        alertDialog(t('composer.pdf_parse_failed', { name: f.name, msg: e instanceof Error ? e.message : String(e) }) + stack)
      } finally {
        setExtracting((n) => n - 1)
      }
    }
  ...
  return (
    <div className="rounded-2xl border border-line bg-white p-3 shadow-card">
        ...
        <div className="mb-2 flex flex-wrap gap-2">
          {docs.map((d) => (
            <span key={d.id} className="flex items-center gap-1.5 rounded-lg border border-line bg-panel px-2.5 py-1.5 text-[12.5px] text-ink-soft">
              ...
              <span className="text-ink-faint">{t('composer.doc_pages', { n: d.pages })}</span>
          ...
          {extracting > 0 && (
            <span className="flex items-center gap-1.5 rounded-lg border border-dashed border-line px-2.5 py-1.5 text-[12.5px] text-ink-muted">
              <Loader2 size={13} className="animate-spin" /> {t('composer.parsing_pdf')}
            </span>
          )}
        </div>
      )}
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        {/* Chat projects have no design system — hide the picker entirely. */}
        <div className="relative" style={project.category === 'Other' ? { display: 'none' } : undefined}>
          <button
            onClick={() => setDsMenu((v) => !v)}
            className="flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-[13px] text-ink-soft hover:bg-panel"
            title={t('composer.ds_section')}
          >
          ...
          {dsMenu && (
            <>
              ...
              <div className="absolute bottom-full left-0 z-30 mb-1 w-56 rounded-xl border border-line bg-white p-1 shadow-pop">
                <div className="px-3 pb-1 pt-1.5 text-[10.5px] font-semibold uppercase tracking-wide text-ink-faint">{t('composer.ds_section')}</div>
                {(
                  [
                    {
                      key: 'default',
                      label: dss.systems.find((s) => s.id === dss.defaultId)?.name
                        ? t('composer.ds_follow_default_named', { name: dss.systems.find((s) => s.id === dss.defaultId)!.name })
                        : t('composer.ds_follow_default_none'),
                      checked: project.designSystemId === undefined,
                      pick: undefined as string | null | undefined,
                    },
                    { key: 'none', label: t('composer.ds_none'), checked: project.designSystemId === null, pick: null },
                    ...dss.systems.map((s) => ({
                      key: s.id,
                      label: s.name || t('composer.ds_unnamed'),
                ...
                {dss.systems.length === 0 && (
                  <div className="px-3 py-1.5 text-[12px] leading-relaxed text-ink-muted">{t('composer.no_ds_hint')}</div>
                )}
              </div>
            </>
          )}
          ...
          {menu && (
            <>
              ...
              <div className="absolute bottom-full right-0 z-30 mb-1 w-60 rounded-xl border border-line bg-white p-1 shadow-pop">
                <div className="px-3 pb-1 pt-1.5 text-[10.5px] font-semibold uppercase tracking-wide text-ink-faint">{t('composer.api_models')}</div>
                {models.map((m) => (
                  <button
                    key={m.id}
                    onClick={() => {
                ...
                {models.length === 0 && (
                  <div className="px-3 py-1.5 text-[12px] leading-relaxed text-ink-muted">{t('composer.no_models_hint')}</div>
                )}
                <div className="px-3 pb-1 pt-2 text-[10.5px] font-semibold uppercase tracking-wide text-ink-faint">{t('composer.cli_agents')}</div>
                {cliAgents.map((c) => (
                  <button
                    key={c.id}
                    onClick={() => {
                ...
                {cliAgents.length === 0 && (
                  <div className="px-3 py-1.5 text-[12px] leading-relaxed text-ink-muted">{t('composer.no_cli_hint')}</div>
                )}
              </div>
            </>
          )}
